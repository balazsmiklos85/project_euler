fn main() {
    ...
    for x in 0..19 {
        for y in 0..19 {
            if y <= 16 {
                let horizontal_product =
                    GRID[x][y] * GRID[x][y + 1] * GRID[x][y + 2] * GRID[x][y + 3];
                max_product = max_product.max(horizontal_product);
            }
            if x <= 16 {
                let vertical_product =
                    GRID[x][y] * GRID[x + 1][y] * GRID[x + 2][y] * GRID[x + 3][y];
                max_product = max_product.max(vertical_product);
            }
            if x <= 16 && y <= 16 {
                let diagonal_product =
                    GRID[x][y] * GRID[x + 1][y + 1] * GRID[x + 2][y + 2] * GRID[x + 3][y + 3];
                max_product = max_product.max(diagonal_product);
                let reverse_diagonal_product =
                    GRID[x][y + 3] * GRID[x + 1][y + 2] * GRID[x + 2][y + 1] * GRID[x + 3][y];
                max_product = max_product.max(reverse_diagonal_product);
            }
        }
    }
    println!("{}", max_product);
